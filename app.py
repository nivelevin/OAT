...
elif (averager.input_value >= config["thresholds"]["combined_limit"]["temp"] and averager.vel >= config["thresholds"]["combined_limit"]["vel"]):
    st.write(f"Temperature {averager.input_value} degree and velocity {averager.vel} kmph. Exceeded! Stopping the process")
    averager.process_enabled = False
else:
...
max_len=max(len(averager.raw_values),len(averager.velocity),len(averager.avg_history),len(averager.first_avg_history))
...
for i in range(len(raw_values)):
    ...
    adjusted_index=i-4
    if i >= 18 and (adjusted_index // 15) < len(avg_history):
        avg = avg_history[adjusted_index //15]
    else:
        avg='-'

    if i >=18 and (adjusted_index//15) < len(first_avg_history):
...
recent_values = raw_values_to_display[-window_size:] if len(raw_values_to_display) >= window_size else raw_values_to_display
...
fig,ax = plt.subplots()
ax.plot(averager.raw_values[-100:],label='Input Value')
ax.plot(averager.velocity[-100:],label='Input Velocity')
